...
class MyHostApduService : HostApduService() {
  ...
  private val TAG = "JDR HostApduService"
  ...
  private val READ_CAPABILITY_CONTAINER_RESPONSE = byteArrayOf(
      ...
      0x00.toByte(), 0x32.toByte(), // Maximum NDEF file size of 50 bytes
  ...
  private var NDEF_URI_LEN = BigInteger.valueOf(NDEF_URI_BYTES.size.toLong()).toByteArray()
  ...
  override fun onStartCommand(intent: Intent, flags: Int, startId: Int): Int {
    ...
    if (intent.hasExtra("ndefMessage")) {
      ...
      NDEF_URI_LEN = BigInteger.valueOf(NDEF_URI_BYTES.size.toLong()).toByteArray()

      val context = applicationContext
      val text = "Your NDEF text has been set!"
  ...
  override fun processCommandApdu(commandApdu: ByteArray, extras: Bundle?): ByteArray {
    ...
    if (Arrays.equals(NDEF_READ_BINARY_NLEN, commandApdu)) {
      ...
      // Build our response
      val response = ByteArray(start.size + NDEF_URI_LEN.size + A_OKAY.size)
      System.arraycopy(start, 0, response, 0, start.size)
      System.arraycopy(NDEF_URI_LEN, 0, response, start.size, NDEF_URI_LEN.size)
      System.arraycopy(A_OKAY, 0, response, start.size + NDEF_URI_LEN.size, A_OKAY.size)
    ...
    if (Arrays.equals(commandApdu.sliceArray(0..1), NDEF_READ_BINARY)) {
      ...
      val fullResponse = ByteArray(start.size + NDEF_URI_LEN.size + NDEF_URI_BYTES.size)

      System.arraycopy(start, 0, fullResponse, 0, start.size)
      System.arraycopy(NDEF_URI_LEN, 0, fullResponse, start.size, NDEF_URI_LEN.size)
      System.arraycopy(NDEF_URI_BYTES, 0, fullResponse, start.size + NDEF_URI_LEN.size, NDEF_URI_BYTES.size)
